...
def buscar_partidas_por_time(nome_time):
    ...
    if "events" in dados:
        for e in dados["events"]:
            if e.get("tournament", {}).get("category", {}).get("sport", {}).get("name") == "Football":
                partidas.append({
                    "id": e["id"],
                    "time_casa": e["homeTeam"]["name"],
                    "time_fora": e["awayTeam"]["name"],
                    "data": e["startTimestamp"]
                })
    return partidas
...
def main():
    ...
    if st.button("Buscar Jogos"):
        partidas = buscar_partidas_por_time(nome_time)
        if not partidas:
            st.warning("Nenhuma partida encontrada.")
            return

        partida = partidas[0]
        st.subheader(f"🆚 {partida['time_casa']} vs {partida['time_fora']}")
